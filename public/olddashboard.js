...
function createReimbursementCardComponent(reimbursement){
  ...
  divCard.setAttribute('id',reimbursement.id);
  ...
  let divContent = createElement('div','content',`<p>Amount: ${reimbursement.amount}</p> <p>${isResolved(reimbursement)}</p> <p>Status: ${reimbursement.status}</p> <p>Type: ${reimbursement.type}</p>`);
  ...
  if(localStorage.getItem('userRole') == 1){
    ...
    let edit = createElement('button','card-footer-item');
      edit.addEventListener('click',(reimbursement)=>{
        editReimbursement(reimbursement.id);
      }
    );
...
function isResolved(reimbursement){
  return reimbursement.resolvedTime ? `Submitted Time: ${reimbursement.submittedTime}  Resolved Time: ${reimbursement.resolvedTime}` : `Submitted Time: ${reimbursement.submittedTime}`; 
}
